...
$(document).ready(function() {
	...
	socket.on('overallStats', function(data){
		...
		$("#ouiq").text(data.tcuq);
		$("#ouas").text(data.tcua);
		$("#ocunavail").text(Math.round(((data.tcun+data.tco)/data.tco)*100) + "%");
...
//		for(cnt = 0; cnt < Object.keys(ddata).length; cnt++)
		...
		for(var i in ddata)
		{
			...
			if(rowid === null)		// row doesnt exist so create one
			{
				...
				col = row.insertCell(11).innerHTML = Math.round(((ddata[i].tcun+ddata[i].tco)/ddata[i].tco)*100) +"%";
			...
			else
			{
				...
				rowid.cells[11].innerHTML = Math.round(((ddata[i].tcun+ddata[i].tco)/ddata[i].tco)*100) +"%";
